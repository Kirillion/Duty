...
def decision_tree(X, Y, scale, level=0):
    ...
    for i in range(n):
        ...
        if scale[i] == CATEGORICAL:   # категориальный признак
            ...
            info_s = 0
            podmn = np.unique(X[:,i])
            for i2 in range(len(np.unique(X[:,i]))):
                indexes = np.where(X[:, i] == podmn[i2])
                Tj = Y[indexes]
                info_s += len(Tj) / len(X[:, i]) * Info(Tj)
...
def Info(set):
    m = len(set)
    info = 0
    for i in np.unique(set):
        info -= (np.sum(i == set)/m) * np.log2(np.sum(i == set)/m)
